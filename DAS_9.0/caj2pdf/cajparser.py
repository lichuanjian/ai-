...
class CAJParser(object):
    ...
    def _convert_caj(self, dest):
        ...
        # Use mutool to repair xref when available.  In this environment,
        # PyPDF2/PyMuPDF can often read the generated file directly, so keep
        # the raw output as a fallback when mutool is not installed.
        try:
    ...
    def _convert_kdh(self, dest):
        ...
        # Use mutool to repair xref when available; otherwise retain the
        # decrypted PDF, which is readable by modern PDF parsers.
        try:
